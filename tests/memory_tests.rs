...
fn test_rom_read() {
    ...
    // Para testar banco 0 ($00:8000), definimos ROM[0x0000]
    rom[0x0000] = 0x12; // Mapeia para $00:8000
    rom[0x0001] = 0x34; // Mapeia para $00:8001
    
    // Para testar banco 1 ($01:8000), definimos ROM[0x8000] 
    rom[0x8000] = 0x56; // Mapeia para $01:8000
    rom[0x8001] = 0x78; // Mapeia para $01:8001
    
    let memory = Memory::new(rom);
    
    // Teste leitura ROM banco 0
    assert_eq!(memory.read(0x008000), 0x12);
    assert_eq!(memory.read(0x008001), 0x34);
    
    // Teste leitura ROM banco 1  
    assert_eq!(memory.read(0x018000), 0x56);
    assert_eq!(memory.read(0x018001), 0x78);
    
    // Teste leitura ROM em banco 80 (mirror do banco 0)
